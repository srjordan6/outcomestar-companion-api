...
Three endpoints
  POST v1authextension-pair       redeem pairing code - api_token
  GET  v1extensionenrollments     list student_external_identifiers for token's tenant
  POST v1extensionexternal-data-import  receive scraped payload, insert events
...
v0.4.0 (2026-06-26) initial. SwimCloud + USA Swimming content scripts.
...
async def db_conn(tenant_id Optional[str] = None)
    ...
    async with pool.acquire() as conn
        if tenant_id
            await conn.execute(fSET app.current_tenant_id = '{tenant_id}')
        yield conn
...
# ===== Parsers =====
def parse_event(raw_event str) - Optional[dict]
    if not raw_event
        return None
    s = raw_event.strip()
...
def parse_time(raw_time str) - Optional[float]
    if not raw_time
        return None
...
def parse_improvement(s Optional[str]) - Optional[float]
    if not s
        return None
...
def parse_meet_dates(s Optional[str]) - tuple[Optional[date], Optional[date]]
    ...
    range_match = re.match(r^([A-Za-z]+)s+(d{1,2})s[-u2013u2014]s(d{1,2}),sd{4}$, s)
    if range_match
    ...
    single_match = re.match(r^([A-Za-z]+)s+(d{1,2}),sd{4}$, s)
    if single_match
...
def normalize_round(raw_round Optional[str]) - Optional[str]
    if not raw_round
        return None
...
def normalize_place(raw_place Optional[str]) - Optional[int]
    if not raw_place
        return None
    m = re.match(r(d+), raw_place.strip())
    return int(m.group(1)) if m else None

def build_source_id(meet_start Optional[date], distance int, stroke_short str,
                    course_long Optional[str], swim_time str,
                    round_normalized Optional[str], is_relay bool) - str
    YYYYMMDD_DDstroke_COURSE_TIME[rp]. Matches existing 182-row convention.
...
# ===== Auth =====
async def authenticate(authorization Annotated[Optional[str], Header()] = None) - dict
    ...
    async with db_conn() as conn
        row = await conn.fetchrow(
            SELECT tenant_id, student_ids, scope, revoked_at, expires_at
...
@router.post(v1authextension-pair, response_model=PairResponse, tags=[extension])
async def extension_pair(req PairRequest)
    ...
    async with db_conn() as conn
        row = await conn.fetchrow(
            SELECT tenant_id, student_ids, name, expires_at, redeemed_at, created_by
        ...
        token_id = await conn.fetchval(
            INSERT INTO api_tokens
                 (tenant_id, token_hash, student_ids, name, scope, created_by, expires_at)
               VALUES ($1, $2, $3, $4, 'extension', $5, $6)
               RETURNING id,
        ...
        await conn.execute(
            UPDATE pairing_codes
               SET redeemed_at = now(), redeemed_token_id = $1
               WHERE code = $2,
            token_id, code,
        )
...
@router.get(v1extensionenrollments, response_model=list[EnrollmentItem], tags=[extension])
async def get_enrollments(auth dict = Depends(authenticate))
    async with db_conn(tenant_id=auth[tenant_id]) as conn
        rows = await conn.fetch(
            SELECT student_id, system_name, external_id, external_url,
                      is_primary, last_synced_at, last_sync_status
               FROM student_external_identifiers
               WHERE student_id = ANY($1uuid[])
                 AND deleted_at IS NULL
               ORDER BY student_id, system_name,
            [auth[student_ids]],
        )
    return [
        EnrollmentItem(
...
@router.post(v1extensionexternal-data-import, response_model=ImportResponse, tags=[extension])
async def import_external_data(req ImportRequest, auth dict = Depends(authenticate))
    ...
    async with db_conn(tenant_id=auth[tenant_id]) as conn
        student_row = await conn.fetchrow(
            SELECT student_id FROM student_external_identifiers
               WHERE system_name = $1 AND external_id = $2
                 AND deleted_at IS NULL
                 AND student_id = ANY($3uuid[])
               LIMIT 1,
        ...
        if not student_row
            raise HTTPException(
                status_code=404,
                detail=fNo enrollment for system={req.system_name} external_id={req.external_id},
            )
        ...
        existing_rows = await conn.fetch(
            SELECT source_id FROM events
               WHERE student_id = $1uuid AND source_system = $2
        ...
        for r in req.races
            try
                ...
                if not ev
                    errors.append(funparseable event {r.raw_event!r})
                    continue
                ...
                source_id = build_source_id(
                    start_d, ev[distance_m], ev[stroke_short],
                    ev[course_long], r.raw_time, round_norm, ev[is_relay],
                )
                ...
                errors.append(frace {r.raw_event!r} {type(e).__name__} {e})

        if to_insert
            async with conn.transaction()
                await conn.executemany(
                    INSERT INTO events
                         (tenant_id, student_id, event_type, title,
                          event_date, event_end_date, location_name,
                          details, visibility, source_system, source_id, created_by)
                       VALUES ($1uuid, $2uuid, $3event_type_enum, $4,
                               $5, $6, $7,
                               $8jsonb, $9visibility_enum, $10, $11, $12uuid),
                    to_insert,
                )
        ...
        await conn.execute(
            UPDATE student_external_identifiers
               SET last_synced_at = now(),
                   last_sync_status = $1, last_sync_summary = $2,
                   updated_at = now()
               WHERE student_id = $3uuid AND system_name = $4 AND external_id = $5,
            status_value,
            fExtension v{req.extractor_version or ''} {inserted} inserted, {skipped} skipped, {len(errors)} errors,
        ...
        if req.raw_html_archive
            await conn.execute(
                INSERT INTO archive_entries
                     (tenant_id, archive_type, archive_date, version, title, summary, detail,
                      source, source_id, visibility, created_by)
                   VALUES ($1uuid, 'client_scrape_html', CURRENT_DATE, $2, $3, $4, $5,
                           'outcomestar_companion', $6, 'private', $7uuid),
                auth[tenant_id], req.extractor_version or unknown,
                fClient scrape {req.system_name} {req.external_id},
                fCaptured {inserted} new races, {skipped} duplicates skipped,
                req.raw_html_archive,
                fclient_scrape_{req.system_name}_{req.external_id}_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')},
                STEPHEN_USER_ID,
            )
